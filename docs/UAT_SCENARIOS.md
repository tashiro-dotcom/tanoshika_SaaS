...
1. 「認証状態: ログイン中」が表示される
2. エラーメッセージが表示されない
3. `challenge_invalid_or_expired` 発生時は「ログイン（MFA開始）」から再実行して復旧できる
...
### 手順
1. 年月を指定して「月次賃金を計算」を押す
2. 計算IDを指定して「賃金を承認」を押す
...
2. MFAチャレンジ期限切れ: 「ログインからやり直し」文言が表示され、再ログインで復旧できること
3. 権限不足操作: 「権限がありません」系の文言が表示されること
4. 不正ID入力: 「対象データが見つかりません」またはバリデーションエラーが表示されること
